...
class EfficientNetV2STrainer:
    """EfficientNet V2-S 训练器"""
    ...
    def __init__(self, config):
        ...
        # 创建实验目录
        self.experiment_dir = Path(f"experiments/experiment_{datetime.now().strftime('%Y%m%d_%H%M%S')}/efficientnet_v2_s")
    ...
    def create_model(self):
        """创建模型"""
        model = EfficientNetV2S(num_classes=self.config['num_classes'])
    ...
    def train(self):
        """主训练循环"""
        logging.info("开始训练 EfficientNet V2-S...")
...
def main():
    ...
    # 创建训练器
    trainer = EfficientNetV2STrainer(config)
