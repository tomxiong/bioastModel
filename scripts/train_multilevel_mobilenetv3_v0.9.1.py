#!/usr/bin/env python3
"""
训练 Multilevel MobileNetV3 v0.9.1

版本说明:
- 使用修复后的 Interference F1 指标
- 使用固定数据集划分 (seed=42)
- 基于原版 Multilevel MobileNetV3 配置
- 训练完整的 20 epochs

改进点:
1. ✅ 修复 Interference 指标计算（F1 代替准确率）
2. ✅ 使用固定数据集划分
3. ✅ 完整训练（20 epochs）
"""
...
def main():
    parser = argparse.ArgumentParser(
        description='Train Multilevel MobileNetV3 v0.9.1',
    ...
    parser.add_argument('--experiment-dir', type=str,
                       default='experiments/multilevel_mobilenetv3_v0.9.1',
                       help='实验目录')
    ...
    config['description'] = 'Multilevel MobileNetV3 with fixed F1 metric and dataset split'

    with open(experiment_dir / 'config.json', 'w', encoding='utf-8') as f:
        json.dump(config, f, indent=2, ensure_ascii=False)

    print("="*80)
    print("Multilevel MobileNetV3 v0.9.1 训练")
    print("="*80)
    print(f"版本: v0.9.1")
    ...
    print(f"\n改进点:")
    print(f"  ✅ 使用 F1 分数评估 Interference 任务")
    print(f"  ✅ 使用固定数据集划分 (seed={args.seed})")
    print(f"  ✅ 完整训练 ({args.num_epochs} epochs)")
    print("="*80)
    ...
    model_info = {
        'model_name': 'Multilevel MobileNetV3 v0.9.1',
        'model_size': args.model_size,
    ...
    print(f"  - best_checkpoint.pth: 最佳检查点")
    print("="*80)
