#!/usr/bin/env python3
"""
训练 Multilevel MobileNetV3 v0.10.0

版本说明:
- 基于 v0.9.9，实施 Pattern-Conditional Pores Loss
- 重点解决: 代理学习问题，让模型在业务关键样本上学习 pores

业务需求（已澄清）:
1. Negative 样本：需要检测 pores（支持 negative 判断）
   - 797/863 (92.4%)
2. Positive 关键 Pattern：需要检测 pores（发现误判）
   - center_dots + weak_scattered_pos: 66/863 (7.6%)

v0.9.9 问题:
- Pores Recall: 20.0% (172/864)
- 业务关键 Recall: 19.9% (172/863)
- 差距: 55.1% (需额外检测 475 个样本)

改进点 (v0.10.0):
1. 🆕 **Pattern-Conditional Pores Loss** (核心创新)
   - Negative 样本: 权重 15.0
   - Positive 关键 pattern: 权重 15.0
   - 其他 Positive: 权重 0.1 (几乎忽略)

2. ✅ 继承 v0.9.9 其他配置
   - 任务权重: [1.0, 2.0, 1.5] (提升 interference)
   - 类别权重: [3.0, 5.0, 10.0, 8.0]
   - 训练轮数: 40 epochs

目标 (v0.10.0):
- **Pores Recall**: 20% → **60-75%** (提升 40-55%)
- **业务关键 Recall**: >= 60%
- **Pores F1**: 28.57% → **45-60%**
- 其他任务保持: Level 98%+, Pattern 80%+
"""
...
def main():
    parser = argparse.ArgumentParser(
        description='Train Multilevel MobileNetV3 v0.10.0 (Pattern-Conditional Pores Loss)',
    ...
    # v0.10.0: Pattern-Conditional Loss 参数
    ...
    parser.add_argument('--task-weights', type=float, nargs=3,
                       default=[1.0, 2.0, 1.5],
                       help='任务权重 [growth_level, growth_pattern, interference] (v0.10.0: 提升 interference)')
    ...
    parser.add_argument('--experiment-dir', type=str,
                       default='experiments/multilevel_mobilenetv3_v0.10.0',
                       help='实验目录')
    ...
    config['description'] = 'Pattern-Conditional Pores Loss: 针对业务需求优化 pores 检测'

    with open(experiment_dir / 'config.json', 'w', encoding='utf-8') as f:
        json.dump(config, f, indent=2, ensure_ascii=False)

    print("="*80)
    print("Multilevel MobileNetV3 v0.10.0 Training")
    print("="*80)
    print(f"Version: v0.10.0 (Pattern-Conditional Pores Loss)")
    print(f"Experiment dir: {experiment_dir}")
    print(f"Device: {device}")
    print(f"\nCore Innovation (v0.10.0):")
    ...
    print(f"\nBusiness Logic:")
    print(f"  - Negative pores: 797/863 (92.4%) - 需要检测")
    print(f"  - Positive critical pores: 66/863 (7.6%) - 需要检测")
    print(f"  - Other pores: 1/864 (0.1%) - 可忽略")
    print(f"\nTarget (v0.10.0):")
    print(f"  - Pores Recall: 20% → 60-75%")
    print(f"  - Pores F1: 28.57% → 45-60%")
    print(f"  - Business Critical Recall >= 60%")
    print("="*80)
    ...
    print(f"  Model: Multilevel MobileNetV3 ({args.model_size})")
