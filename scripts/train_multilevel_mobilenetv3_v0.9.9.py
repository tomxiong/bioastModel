#!/usr/bin/env python3
"""
训练 Multilevel MobileNetV3 v0.9.9

版本说明:
- 基于 v0.9.8，针对 pores 检测失败问题调整训练配置
- 重点解决: pores 类别权重过低导致模型完全未学习 pores 特征

v0.9.8 问题诊断:
- 数据质量: ✅ 92.7% pores 纯净度（优秀）
- 样本量: ✅ 测试集 864 个 pores 样本（28.77%）
- 模型性能: ❌ Pores F1 0%（Precision 0%, Recall 0%）
- 根本原因: ❌ pores 类别权重 1.0（最低），任务权重 0.8（最低）

改进点 (v0.9.9):
1. 🆕 **调整 Interference 类别权重** (关键改进)
   - artifacts: 3.0 (保持)
   - contamination: 5.0 (保持)
   - debris: 50.0 → 10.0 (降低，过高导致不稳定)
   - pores: 1.0 → 8.0 (大幅提升 8 倍)

2. 🆕 **提升 Interference 任务权重**
   - 任务权重: [1.0, 2.0, 0.8] → [1.0, 2.0, 1.2]
   - Interference 从最低优先级 → 中等优先级

3. 🆕 **增加训练轮数**
   - 训练轮数: 35 → 40 epochs (确保充分学习)
   - 早停耐心: 15 epochs (保持)

4. ✅ 继承 v0.9.8 数据集
   - 使用第二轮清理后的数据集 (m9e1n170_cleaned_round2.json)
   - 使用固定数据集划分 (dataset_split_seed44.json)
   - Pores 纯净度: 92.7% (Negative 占比)

核心优化:
- **数据集**: 第二轮清理后（pores 纯净度 92.7%）
- **任务权重**: [1.0, 2.0, 1.2] (提升 interference 0.8→1.2)
- **类别权重**: [3.0, 5.0, 10.0, 8.0] (提升 pores 1.0→8.0, 降低 debris 50.0→10.0)
- **阈值优化**: 启用
- **训练轮数**: 40 epochs (增加 5 epochs)
- **早停耐心**: 15

目标 (v0.9.9):
- **pores F1**: 0% → **30-50%** (权重调整预期改善)
- **pores 精确率**: **40-60%**
- **pores 召回率**: **30-50%**
- **Interference 整体 F1**: 48.36% → **55-60%** (整体提升 7-12%)
- **Growth Pattern**: 保持 84%+
- **Growth Level**: 保持 98%+

对比 v0.9.8:
- v0.9.8: 数据清理策略（未达预期，pores F1 仍为 0%）
- v0.9.9: 训练配置优化（针对性解决权重问题）
"""
...
def main():
    parser = argparse.ArgumentParser(
        description='Train Multilevel MobileNetV3 v0.9.9 (Adjusted Pores Weight)',
    ...
    parser.add_argument('--num-epochs', type=int, default=40,
                       help='训练轮数 (v0.9.9: 35→40, 确保充分学习 pores)')
    ...
    parser.add_argument('--interference-weights', type=float, nargs=4,
                       default=[3.0, 5.0, 10.0, 8.0],
                       help='Interference类别权重 [artifacts, contamination, debris, pores] (v0.9.9: pores 1.0→8.0, debris 50.0→10.0)')
    parser.add_argument('--use-class-weights', action='store_true', default=True,
                       help='使用类别权重')

    # v0.9.9: 任务权重参数（提升 interference）
    parser.add_argument('--task-weights', type=float, nargs=3,
                       default=[1.0, 2.0, 1.2],
                       help='任务权重 [growth_level, growth_pattern, interference] (v0.9.9: interference 0.8→1.2)')
    ...
    parser.add_argument('--experiment-dir', type=str,
                       default='experiments/multilevel_mobilenetv3_v0.9.9',
                       help='实验目录')
    ...
    config['version'] = 'v0.9.9'
    config['description'] = 'Multilevel MobileNetV3 v0.9.9: Adjusted pores weight (1.0→8.0) and interference task weight (0.8→1.2)'

    with open(experiment_dir / 'config.json', 'w', encoding='utf-8') as f:
        json.dump(config, f, indent=2, ensure_ascii=False)

    print("="*80)
    print("Multilevel MobileNetV3 v0.9.9 训练")
    print("="*80)
    print(f"版本: v0.9.9 (调整 Pores 权重)")
    print(f"实验目录: {experiment_dir}")
    print(f"设备: {device}")
    print(f"标注文件: {args.annotations_file}")
    print(f"固定划分: {args.split_file}")
    print(f"\n🔍 v0.9.8 问题诊断:")
    print(f"  ✅ 数据质量: 92.7% pores 纯净度（优秀）")
    print(f"  ✅ 样本量: 测试集 864 个 pores 样本（28.77%）")
    print(f"  ❌ 模型性能: Pores F1 0% (Precision 0%, Recall 0%)")
    print(f"  ❌ 根本原因: pores 类别权重 1.0（最低），任务权重 0.8（最低）")
    print(f"\n🆕 核心改进 (v0.9.9):")
    print(f"  ⭐ Interference 类别权重: {args.interference_weights}")
    print(f"     - pores: 1.0 → 8.0 (提升 8 倍)")
    print(f"     - debris: 50.0 → 10.0 (降低，避免过度)")
    print(f"  ⭐ 任务权重: {args.task_weights}")
    print(f"     - interference: 0.8 → 1.2 (提升优先级)")
    print(f"  ⭐ 训练轮数: 35 → {args.num_epochs} epochs")
    print(f"  ✅ 阈值优化: 启用")
    print(f"  ✅ 早停耐心: {args.patience} epochs")
    print(f"\n🎯 v0.9.9 目标 (基于权重调整):")
    print(f"  - pores F1: 0% → 30-50% (权重调整预期改善)")
    print(f"  - pores 精确率: 40-60%")
    print(f"  - pores 召回率: 30-50%")
    print(f"  - Interference 整体 F1: 48.36% → 55-60% (提升 7-12%)")
    print(f"  - Growth Pattern: 保持 84%+")
