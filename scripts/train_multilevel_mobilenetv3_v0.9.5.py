#!/usr/bin/env python3
"""
训练 Multilevel MobileNetV3 v0.9.5

版本说明:
- 基于v0.9.4，优化任务平衡性
- 目标: 恢复 Growth Pattern 性能 + 保持 Interference F1 50%+

改进点:
1. ✅ 继承 v0.9.4 的所有优化 (类别权重 + 阈值优化)
2. 🆕 缩短训练时间 - 50→35 epochs (基于v0.9.4最佳epoch=21的发现)
3. 🆕 提高 Growth Pattern 权重 - 1.5→2.0
4. 🆕 调整早停耐心值 - 20→15 epochs

核心优化:
- 任务权重: [1.0, 2.0, 0.8] (v0.9.4: [1.0, 1.5, 0.8])
- 类别权重: [3.0, 5.0, 20.0, 1.0] (继承)
- 阈值优化: 启用 (继承)
- 训练轮数: 35 epochs (v0.9.4: 50)
- 早停耐心: 15 (v0.9.4: 20)

目标:
- Growth Pattern: 80.57% → 85%+
- Interference F1: 保持 50%+ (优化阈值)
- Growth Level: 保持 98%+
- 整体平衡: 三个任务均达到优秀水平
"""
...
def main():
    parser = argparse.ArgumentParser(
        description='Train Multilevel MobileNetV3 v0.9.5',
    ...
    parser.add_argument('--experiment-dir', type=str,
                       default='experiments/multilevel_mobilenetv3_v0.9.5',
                       help='实验目录')
    ...
    config['description'] = 'Multilevel MobileNetV3 with optimized task balance (35 epochs, task_weights=[1.0,2.0,0.8])'

    with open(experiment_dir / 'config.json', 'w', encoding='utf-8') as f:
        json.dump(config, f, indent=2, ensure_ascii=False)

    print("="*80)
    print("Multilevel MobileNetV3 v0.9.5 训练")
    print("="*80)
    print(f"版本: v0.9.5")
    ...
    print(f"  📈 目标:")
    print(f"     - Growth Pattern: 80.57% → 85%+")
    print(f"     - Interference F1: 保持 50%+ (优化阈值)")
    print(f"     - Growth Level: 保持 98%+")
    print(f"     - 整体平衡: 三个任务均达到优秀水平")
    print("="*80)
