"""
RegNet Y-400MF 使用真实数据训练脚本
基于DenseNet真实数据训练的成功经验
"""
...
from models.regnet_wrapper import RegNetY400MF
...
class RegNetRealDataTrainer:
    """RegNet Y-400MF 真实数据训练器"""
    ...
    def __init__(self, config):
        ...
        # 创建实验目录
        self.experiment_dir = Path(f"experiments/experiment_{datetime.now().strftime('%Y%m%d_%H%M%S')}/regnet_y400mf_real")
    ...
    def create_model(self):
        """创建模型"""
        model = RegNetY400MF(num_classes=self.config['num_classes'])
    ...
    def train(self):
        """主训练循环"""
        logging.info("开始训练 RegNet Y-400MF (真实数据)...")
        ...
        # 创建早停和检查点保存器
        early_stopping = EarlyStopping(patience=12, min_delta=0.001)  # RegNet需要更多patience
...
def main():
    """主函数"""
    # 训练配置 - 基于DenseNet成功经验调整
    config = {
        'model_name': 'regnet_y400mf_real',
        'num_classes': 2,
        'input_size': 70,
        'batch_size': 32,  # 真实数据可以用更大的batch size
        'epochs': 35,      # RegNet可能需要更多轮次
        'learning_rate': 0.001,
        'weight_decay': 1e-4,
    ...
    # 创建训练器
    trainer = RegNetRealDataTrainer(config)
