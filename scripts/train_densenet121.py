...
class DenseNetTrainer:
    ...
    def __init__(self, config):
        ...
        # 创建实验目录
        self.experiment_dir = Path(f"experiments/experiment_{datetime.now().strftime('%Y%m%d_%H%M%S')}/densenet121")
    ...
    def train(self):
        """主训练循环"""
        logging.info("开始训练 DenseNet-121...")
        ...
        # 创建早停和检查点保存器
        early_stopping = EarlyStopping(patience=15, min_delta=0.001)  # DenseNet需要更多耐心
...
def main():
    ...
    config = {
        ...
        'num_classes': 2,
        'input_size': 70,
        'batch_size': 24,  # DenseNet内存占用较大，减小batch size
        'epochs': 50,
        'learning_rate': 0.001,
        'weight_decay': 1e-4,
