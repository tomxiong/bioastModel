...
class GhostNetTrainer:
    ...
    def __init__(self, config):
        ...
        # 创建实验目录
        self.experiment_dir = Path(f"experiments/experiment_{datetime.now().strftime('%Y%m%d_%H%M%S')}/ghostnet")
    ...
    def create_model(self):
        """创建模型"""
        model = GhostNetWrapper(num_classes=self.config['num_classes'])
    ...
    def train(self):
        ...
        # 创建早停和检查点保存器
        early_stopping = EarlyStopping(patience=12, min_delta=0.001)  # GhostNet需要更多耐心
...
def main():
    ...
    # 创建训练器
    trainer = GhostNetTrainer(config)
