"""
将 MIC MobileNetV3 模型转换为 ONNX 格式。

这个脚本专门用于处理 MIC MobileNetV3 模型的权重加载问题，
...
def convert_model_to_onnx(model_path, output_dir="deployment/onnx_models"):
    ...
    Args:
        ...
        output_dir: 输出目录
    """
    logging.info(f"🚀 开始转换模型: mic_mobilenetv3")
    ...
    try:
        ...
        # 定义输出路径
        onnx_path = os.path.join(output_dir, "mic_mobilenetv3.onnx")
        ...
        logging.info(f"✅ 模型 mic_mobilenetv3 转换并测试成功!")
        
        logging.info(f"\n🎉 模型 mic_mobilenetv3 转换成功!")
        logging.info(f"📁 输出目录: {output_dir}")
        
        return True
    except Exception as e:
        logging.error(f"❌ 模型加载失败: {str(e)}")
        logging.error(f"\n❌ 模型 mic_mobilenetv3 转换失败!")
        return False

def main():
    parser = argparse.ArgumentParser(description="将 MIC MobileNetV3 模型转换为 ONNX 格式")
