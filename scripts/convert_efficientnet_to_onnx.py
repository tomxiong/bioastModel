...
class EfficientNetONNXConverter:
    ...
    def _get_checkpoint_path(self, model_name):
        ...
        alt_paths = [
            ...
            f"experiments/experiment_20250802_140818/{model_name}_best.pth"
        ]
    ...
    def load_model(self):
        ...
        try:
            ...
            # 加载模型权重
            checkpoint_path = self._get_checkpoint_path("efficientnet_b0")
    ...
    def convert(self):
        ...
        if self.test_onnx_model(result["onnx_path"]):
            logger.info(f"✅ 模型 efficientnet_b0 转换并测试成功!")
            return True
        else:
            logger.error("\n❌ 模型测试失败!")
            return False
...
def main():
    parser = argparse.ArgumentParser(description="将EfficientNet-B0模型转换为ONNX格式")
    parser.add_argument("--output_dir", type=str, default="deployment/onnx_models", 
                        help="ONNX模型输出目录")
    args = parser.parse_args()
    
    logger.info("🚀 开始转换EfficientNet-B0模型")
    
    converter = EfficientNetONNXConverter(output_dir=args.output_dir)
    success = converter.convert()
    
    if success:
        logger.info("\n🎉 模型 efficientnet_b0 转换成功!")
        logger.info(f"📁 输出目录: {converter.output_dir}")
    else:
        logger.error("\n❌ 模型 efficientnet_b0 转换失败!")
