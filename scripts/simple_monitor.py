"""
Simple CoAtNet training monitor (without external dependencies).
"""
...
def main():
    ...
    print("🔍 CoAtNet Training Monitor (Simple)")
    print("=" * 50)
    ...
    for file_name, status in files_status.items():
        if status["exists"]:
            ...
            print(f"   ✅ {file_name}: {size_mb:.1f} MB (modified: {status['modified']})")
        else:
    ...
    print("   - Check terminal for real-time progress")
    print("   - Model files are being saved automatically")
