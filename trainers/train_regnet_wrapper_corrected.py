#!/usr/bin/env python3
"""
Corrected Training Script for regnet_wrapper
...
def train_model():
    """Train the regnet_wrapper model with corrected data loading"""
    print(f"🚀 Starting corrected training for regnet_wrapper")
    ...
    try:
        ...
        try:
            ...
            if "regnet_wrapper" == "micro_vit" or "regnet_wrapper" == "vit_tiny":
                model = RegNetWrapper(num_classes=2)
            elif "regnet_wrapper" == "mnasnet":
                model = RegNetWrapper(1.0, num_classes=2)
            elif "regnet_wrapper" == "shufflenet_v2":
                model = RegNetWrapper([4, 8, 4], [24, 48, 96, 192, 1024], num_classes=2)
            elif "regnet_wrapper" == "densenet":
