#!/usr/bin/env python3
"""
Fixed Training Script for regnet_wrapper
Class: Bottleneck
...
# Import the model
try:
    from models.regnet_wrapper import Bottleneck
    print(f"✅ Successfully imported Bottleneck from models.regnet_wrapper")
except ImportError as e:
    print(f"❌ Import error: {e}")
    print(f"🔄 Trying alternative import methods...")
    try:
        import importlib.util
        spec = importlib.util.spec_from_file_location("regnet_wrapper", f"models/regnet_wrapper.py")
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)
        Bottleneck = getattr(module, "Bottleneck")
        print(f"✅ Successfully imported Bottleneck using importlib")
...
def train_model():
    ...
    print(f"🚀 Starting training for regnet_wrapper (Bottleneck)")
    ...
    try:
        ...
        print(f"🏗️  Creating model: Bottleneck")
        ...
        try:
            ...
            try:
                model = Bottleneck(num_classes=2)
                ...
                try:
                    model = Bottleneck(num_classes=2, pretrained=False)
        ...
        print(f'🎉 Training completed for regnet_wrapper')
        print(f'🏆 Best validation accuracy: {best_val_acc:.2f}%')
        
        return True
        
    except Exception as e:
        print(f"❌ Training failed for regnet_wrapper: {e}")
        traceback.print_exc()
        return False
...
if __name__ == "__main__":
    success = train_model()
    if success:
        print(f"✅ regnet_wrapper training completed successfully")
    else:
        print(f"❌ regnet_wrapper training failed")
        sys.exit(1)
