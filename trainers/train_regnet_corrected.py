#!/usr/bin/env python3
"""
Corrected Training Script for regnet
...
def train_model():
    """Train the regnet model with corrected data loading"""
    print(f"🚀 Starting corrected training for regnet")
    ...
    try:
        ...
        model = RegNet(num_classes=2, w_a=36.44, w_0=20, w_m=2.49, d=13, group_w=8)
        ...
        try:
            ...
            if "regnet" == "micro_vit" or "regnet" == "vit_tiny":
                model = RegNet(num_classes=2)
            elif "regnet" == "mnasnet":
                model = RegNet(1.0, num_classes=2)
            elif "regnet" == "shufflenet_v2":
                model = RegNet([4, 8, 4], [24, 48, 96, 192, 1024], num_classes=2)
            elif "regnet" == "densenet":
                from models.densenet import DenseNet
                model = DenseNet(num_classes=2)
            elif "regnet" == "mic_mobilenetv3":
    ...
    for epoch in range(30):
        try:
            ...
            if val_acc > best_val_acc:
                ...
                checkpoint_path = f'checkpoints/regnet_{timestamp}_best.pth'
