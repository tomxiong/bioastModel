#!/usr/bin/env python3
"""
Fixed Training Script for simplified_airbubble_detector
...
try:
    ...
    print(f"✅ Successfully imported SimplifiedAirBubbleDetector from models.simplified_airbubble_detector")
except ImportError as e:
    print(f"❌ Import error: {e}")
    print(f"🔄 Trying alternative import methods...")
    try:
        import importlib.util
        spec = importlib.util.spec_from_file_location("simplified_airbubble_detector", f"models/simplified_airbubble_detector.py")
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)
        SimplifiedAirBubbleDetector = getattr(module, "SimplifiedAirBubbleDetector")
        print(f"✅ Successfully imported SimplifiedAirBubbleDetector using importlib")
...
def train_model():
    """Train the simplified_airbubble_detector model"""
    print(f"🚀 Starting training for simplified_airbubble_detector (SimplifiedAirBubbleDetector)")
    ...
    try:
        ...
        print(f"🏗️  Creating model: SimplifiedAirBubbleDetector")
        ...
        try:
            ...
            try:
                model = SimplifiedAirBubbleDetector(num_classes=2)
                ...
                try:
                    model = SimplifiedAirBubbleDetector(num_classes=2, pretrained=False)
        ...
        for epoch in range(num_epochs):
            ...
            if val_acc > best_val_acc:
                ...
                checkpoint_path = f'checkpoints/simplified_airbubble_detector_{timestamp}_best.pth'
                torch.save({
                    ...
                    'val_acc': val_acc,
                    'train_acc': train_acc,
                    'model_class': 'SimplifiedAirBubbleDetector',
        ...
        print(f'🎉 Training completed for simplified_airbubble_detector')
        print(f'🏆 Best validation accuracy: {best_val_acc:.2f}%')
        
        return True
        
    except Exception as e:
        print(f"❌ Training failed for simplified_airbubble_detector: {e}")
        traceback.print_exc()
        return False
...
if __name__ == "__main__":
    success = train_model()
    if success:
        print(f"✅ simplified_airbubble_detector training completed successfully")
    else:
        print(f"❌ simplified_airbubble_detector training failed")
        sys.exit(1)
