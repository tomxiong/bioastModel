#!/usr/bin/env python3
"""
Corrected Training Script for ghostnet
...
def train_model():
    """Train the ghostnet model with corrected data loading"""
    print(f"🚀 Starting corrected training for ghostnet")
    ...
    try:
        ...
        try:
            ...
            if "ghostnet" == "micro_vit" or "ghostnet" == "vit_tiny":
                model = GhostNet(num_classes=2)
            elif "ghostnet" == "mnasnet":
                model = GhostNet(1.0, num_classes=2)
            elif "ghostnet" == "shufflenet_v2":
                model = GhostNet([4, 8, 4], [24, 48, 96, 192, 1024], num_classes=2)
            elif "ghostnet" == "densenet":
