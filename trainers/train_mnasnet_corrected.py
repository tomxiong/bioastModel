#!/usr/bin/env python3
"""
Corrected Training Script for mnasnet
...
def train_model():
    """Train the mnasnet model with corrected data loading"""
    print(f"🚀 Starting corrected training for mnasnet")
    ...
    try:
        ...
        try:
            ...
            if "mnasnet" == "micro_vit" or "mnasnet" == "vit_tiny":
                model = MNASNet(num_classes=2)
            elif "mnasnet" == "mnasnet":
                model = MNASNet(1.0, num_classes=2)
            elif "mnasnet" == "shufflenet_v2":
                model = MNASNet([4, 8, 4], [24, 48, 96, 192, 1024], num_classes=2)
            elif "mnasnet" == "densenet":
