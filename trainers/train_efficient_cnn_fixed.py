#!/usr/bin/env python3
"""
Fixed Training Script for efficient_cnn
Class: EfficientCnn
...
try:
    ...
    print(f"✅ Successfully imported EfficientCnn from models.efficient_cnn")
except ImportError as e:
    print(f"❌ Import error: {e}")
    print(f"🔄 Trying alternative import methods...")
    try:
        import importlib.util
        spec = importlib.util.spec_from_file_location("efficient_cnn", f"models/efficient_cnn.py")
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)
        EfficientCnn = getattr(module, "EfficientCnn")
        print(f"✅ Successfully imported EfficientCnn using importlib")
...
def train_model():
    """Train the efficient_cnn model"""
    print(f"🚀 Starting training for efficient_cnn (EfficientCnn)")
    ...
    try:
        ...
        print(f"🏗️  Creating model: EfficientCnn")
        ...
        try:
            ...
            try:
                ...
                try:
                    model = EfficientCnn(num_classes=2, pretrained=False)
        ...
        for epoch in range(num_epochs):
            ...
            if val_acc > best_val_acc:
                ...
                checkpoint_path = f'checkpoints/efficient_cnn_{timestamp}_best.pth'
                torch.save({
        ...
        print(f'🎉 Training completed for efficient_cnn')
        print(f'🏆 Best validation accuracy: {best_val_acc:.2f}%')
        
        return True
        
    except Exception as e:
        print(f"❌ Training failed for efficient_cnn: {e}")
        traceback.print_exc()
        return False
...
if __name__ == "__main__":
    success = train_model()
    if success:
        print(f"✅ efficient_cnn training completed successfully")
    else:
        print(f"❌ efficient_cnn training failed")
        sys.exit(1)
