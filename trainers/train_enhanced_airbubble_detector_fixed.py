#!/usr/bin/env python3
"""
Fixed Training Script for enhanced_airbubble_detector
Class: AirBubbleAttentionModule
...
# Import the model
try:
    from models.enhanced_airbubble_detector import AirBubbleAttentionModule
    print(f"✅ Successfully imported AirBubbleAttentionModule from models.enhanced_airbubble_detector")
except ImportError as e:
    print(f"❌ Import error: {e}")
    print(f"🔄 Trying alternative import methods...")
    try:
        import importlib.util
        spec = importlib.util.spec_from_file_location("enhanced_airbubble_detector", f"models/enhanced_airbubble_detector.py")
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)
        AirBubbleAttentionModule = getattr(module, "AirBubbleAttentionModule")
        print(f"✅ Successfully imported AirBubbleAttentionModule using importlib")
...
def train_model():
    """Train the enhanced_airbubble_detector model"""
    print(f"🚀 Starting training for enhanced_airbubble_detector (AirBubbleAttentionModule)")
    ...
    try:
        ...
        print(f"🏗️  Creating model: AirBubbleAttentionModule")
        ...
        try:
            model = AirBubbleAttentionModule()
            ...
            try:
                model = AirBubbleAttentionModule(num_classes=2)
                ...
                try:
                    model = AirBubbleAttentionModule(num_classes=2, pretrained=False)
        ...
        for epoch in range(num_epochs):
            ...
            if val_acc > best_val_acc:
                ...
                checkpoint_path = f'checkpoints/enhanced_airbubble_detector_{timestamp}_best.pth'
                torch.save({
                    ...
                    'val_acc': val_acc,
                    'train_acc': train_acc,
                    'model_class': 'AirBubbleAttentionModule',
        ...
        print(f'🎉 Training completed for enhanced_airbubble_detector')
        print(f'🏆 Best validation accuracy: {best_val_acc:.2f}%')
        
        return True
        
    except Exception as e:
        print(f"❌ Training failed for enhanced_airbubble_detector: {e}")
        traceback.print_exc()
        return False
...
if __name__ == "__main__":
    success = train_model()
    if success:
        print(f"✅ enhanced_airbubble_detector training completed successfully")
    else:
        print(f"❌ enhanced_airbubble_detector training failed")
        sys.exit(1)
