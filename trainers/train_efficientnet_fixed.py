#!/usr/bin/env python3
"""
Fixed Training Script for efficientnet
Class: EfficientNetCustom
...
try:
    ...
    print(f"✅ Successfully imported EfficientNetCustom from models.efficientnet")
except ImportError as e:
    print(f"❌ Import error: {e}")
    print(f"🔄 Trying alternative import methods...")
    try:
        import importlib.util
        spec = importlib.util.spec_from_file_location("efficientnet", f"models/efficientnet.py")
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)
        EfficientNetCustom = getattr(module, "EfficientNetCustom")
        print(f"✅ Successfully imported EfficientNetCustom using importlib")
...
def train_model():
    ...
    print(f"🚀 Starting training for efficientnet (EfficientNetCustom)")
    ...
    try:
        ...
        print(f"🏗️  Creating model: EfficientNetCustom")
        ...
        try:
            ...
            try:
                ...
                try:
                    model = EfficientNetCustom(num_classes=2, pretrained=False)
        ...
        for epoch in range(num_epochs):
            ...
            if val_acc > best_val_acc:
                ...
                checkpoint_path = f'checkpoints/efficientnet_{timestamp}_best.pth'
                torch.save({
        ...
        print(f'🎉 Training completed for efficientnet')
        print(f'🏆 Best validation accuracy: {best_val_acc:.2f}%')
        
        return True
        
    except Exception as e:
        print(f"❌ Training failed for efficientnet: {e}")
        traceback.print_exc()
        return False
...
if __name__ == "__main__":
    success = train_model()
    if success:
        print(f"✅ efficientnet training completed successfully")
    else:
        print(f"❌ efficientnet training failed")
        sys.exit(1)
