#!/usr/bin/env python3
"""
DensenetCompact ONNX Converter
Converts trained DensenetCompact model to ONNX format with performance validation
"""
...
from models.densenet_compact import DensenetCompact
from core.real_data_loader import create_real_data_loaders

def load_latest_checkpoint():
    """Load the latest DensenetCompact checkpoint"""
    checkpoint_dir = "checkpoints"
    densenet_files = [f for f in os.listdir(checkpoint_dir) if f.startswith("densenet_compact_") and f.endswith("_best.pth")]
    
    if not densenet_files:
        raise FileNotFoundError("No DensenetCompact checkpoint found")
    
    # Sort by timestamp and get the latest
    densenet_files.sort(reverse=True)
    latest_checkpoint = os.path.join(checkpoint_dir, densenet_files[0])
...
def main():
    print("=" * 60)
    print("DensenetCompact ONNX Conversion")
    print("=" * 60)
    ...
    onnx_filename = f"densenet_compact_{timestamp}.onnx"
    ...
    report_filename = f"densenet_compact_onnx_conversion_{timestamp}.json"
    ...
    print("\n" + "=" * 60)
    print("✅ DensenetCompact ONNX conversion completed successfully!")
    print("=" * 60)
