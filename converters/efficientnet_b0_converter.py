...
import torch
from models.efficientnet import EfficientNetCustom, create_efficientnet_b0

class EfficientNetB0Converter(ONNXConverterBase):
    """EfficientNetB0模型转换器"""
    
    def __init__(self):
        """初始化转换器"""
        super().__init__("efficientnet_b0")
    ...
    def convert(self):
        ...
        if checkpoint_path is None:
            ...
            # 尝试使用固定路径
            checkpoint_path = Path("experiments/experiment_20250803_020217/efficientnet_b0/best_model.pth")
...
def main():
    """主函数"""
    converter = EfficientNetB0Converter()
    success = converter.convert()
    
    if success:
        logging.info(f"EfficientNetB0模型已成功转换为ONNX格式")
    else:
        logging.error(f"EfficientNetB0模型转换失败")
