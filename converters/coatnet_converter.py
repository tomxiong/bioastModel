...
from models.coatnet import CoAtNet, create_coatnet

class CoAtNetConverter(ONNXConverterBase):
    """CoAtNet模型转换器"""
    ...
    def convert(self):
        ...
        if checkpoint_path is None:
            ...
            # 尝试使用固定路径
            checkpoint_path = Path("experiments/experiment_20250803_014815/coatnet/best_model.pth")
        ...
        # 创建模型实例
        try:
            model = create_coatnet(model_size='small')
            model.eval()
        ...
        # 转换为ONNX格式
        # CoAtNet包含注意力机制，可能需要特殊处理
        success = self.convert_to_onnx(
            model, 
            self.input_shape, 
            opset_version=13,  # 注意力机制可能需要更高的opset版本
            dynamic_axes=True
        )
...
def main():
    """主函数"""
    converter = CoAtNetConverter()
    success = converter.convert()
    
    if success:
        logging.info(f"CoAtNet模型已成功转换为ONNX格式")
    else:
        logging.error(f"CoAtNet模型转换失败")
