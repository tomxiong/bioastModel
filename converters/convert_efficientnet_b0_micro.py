#!/usr/bin/env python3
"""
EfficientnetB0Micro ONNX Converter
Converts trained EfficientnetB0Micro model to ONNX format with performance validation
"""
...
def load_latest_checkpoint():
    """Load the latest EfficientnetB0Micro checkpoint"""
    checkpoint_dir = "checkpoints"
    efficientnet_files = [f for f in os.listdir(checkpoint_dir) if f.startswith("efficientnet_b0_micro_") and f.endswith("_best.pth")]
    
    if not efficientnet_files:
        raise FileNotFoundError("No EfficientnetB0Micro checkpoint found")
    
    # Sort by timestamp and get the latest
    efficientnet_files.sort(reverse=True)
    latest_checkpoint = os.path.join(checkpoint_dir, efficientnet_files[0])
...
def main():
    print("=" * 60)
    print("EfficientnetB0Micro ONNX Conversion")
    print("=" * 60)
    ...
    onnx_filename = f"efficientnet_b0_micro_{timestamp}.onnx"
    ...
    report_filename = f"efficientnet_b0_micro_onnx_conversion_{timestamp}.json"
    ...
    print("\n" + "=" * 60)
    print("✅ EfficientnetB0Micro ONNX conversion completed successfully!")
    print("=" * 60)
