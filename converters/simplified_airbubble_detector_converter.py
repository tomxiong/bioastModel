"""
SimplifiedAirBubbleDetector模型转换器
"""
...
class SimplifiedAirbubbleDetectorConverter(ONNXConverterBase):
    """SimplifiedAirBubbleDetector模型转换器"""
    
    def __init__(self):
        """初始化转换器"""
        super().__init__("simplified_airbubble_detector")
    ...
    def convert(self):
        ...
        if checkpoint_path is None:
            ...
            # 尝试使用固定路径
            checkpoint_path = Path("experiments/experiment_20250803_020217/simplified_airbubble_detector/best_model.pth")
        ...
        # 转换为ONNX格式
        success = self.convert_to_onnx(model, self.input_shape, opset_version=11)
...
def main():
    """主函数"""
    converter = SimplifiedAirbubbleDetectorConverter()
    success = converter.convert()
    
    if success:
        logging.info(f"SimplifiedAirBubbleDetector模型已成功转换为ONNX格式")
    else:
        logging.error(f"SimplifiedAirBubbleDetector模型转换失败")
