...
def disable_input_validation(model):
    """Temporarily disable input validation for ONNX export"""
    ...
    if hasattr(model, 'forward'):
        ...
        # Create a wrapper that skips input validation
        def forward_wrapper(x):
            # Skip the input size validation for ONNX export
            if isinstance(model, MICMobileNetV3):
                # MIC MobileNetV3 forward without validation
                x = model.stem(x)
                for block in model.blocks:
                    x = block(x)
                x = model.medical_features(x)
                x = model.final_conv(x)
                x = model.global_pool(x)
                x = x.view(x.size(0), -1)
                x = model.classifier(x)
                return x
            elif isinstance(model, ResnetMicro):
                # ResNet Micro forward without validation
                ...
                x = model.fc(x)
                return x
            elif isinstance(model, InceptionMicro):
                # Inception Micro forward without validation
                x = model.features(x)
                x = x.view(x.size(0), -1)
                x = model.classifier(x)
                return x
            else:
                # For other models, use original forward
                return original_forwards['model'](x)
        
        model.forward = forward_wrapper
    
    return original_forwards

def restore_input_validation(model, original_forwards):
    """Restore original forward methods"""
    if 'model' in original_forwards:
        model.forward = original_forwards['model']
...
def convert_top4_models_to_onnx():
    ...
    for i, config in enumerate(models_config, 1):
        ...
        try:
            ...
            # Disable input validation for ONNX export
            original_forwards = disable_input_validation(model)
            ...
            restore_input_validation(model, original_forwards)
