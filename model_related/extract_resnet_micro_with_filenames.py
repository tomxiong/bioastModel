#!/usr/bin/env python3
"""
Extract error samples from resnet_micro checkpoint with source filenames
"""
...
def create_index_to_filename_mapping():
    ...
    # Create index mapping for validation set
...
def extract_resnet_micro_error_samples():
    """Extract error samples from resnet_micro with source filenames"""
    
    print("🔍 提取resnet_micro错误样本清单 (包含源文件名)...")
    print("=" * 80)
    ...
    # Load checkpoint
    checkpoint_path = "checkpoints/resnet_micro_20250808_005254_best.pth"
    ...
    error_analysis = {
        ...
        'checkpoint_info': {
            ...
            'best_validation_accuracy': float(checkpoint.get('best_val_acc', 0)),
            'final_validation_accuracy': float(checkpoint.get('history', {}).get('val_acc', [0])[-1]) if 'history' in checkpoint else 0,
            'final_validation_loss': float(checkpoint.get('history', {}).get('val_loss', [0])[-1]) if 'history' in checkpoint else 0,
            'has_training_history': 'history' in checkpoint
        },
        'dataset_info': {
            'name': 'bioast_dataset',
        ...
        'training_history': convert_numpy_types(checkpoint.get('history', {}))
    }
    
    # Save detailed report
    os.makedirs('error_analysis', exist_ok=True)
    report_path = f"error_analysis/resnet_micro_with_filenames_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    # Generate summary
    print(f"\n" + "=" * 80)
    print(f"RESNET_MICRO 错误样本分析总结 (包含源文件名)")
    print(f"=" * 80)
    print(f"模型: resnet_micro")
    print(f"数据集: bioast_dataset")
    print(f"Checkpoint: {checkpoint_path}")
    print(f"训练轮数: {checkpoint.get('epoch', 'N/A')}")
    print(f"最佳验证准确率: {checkpoint.get('best_val_acc', 0):.4f}%")
    if 'history' in checkpoint:
        print(f"最终验证准确率: {checkpoint['history']['val_acc'][-1]:.4f}%")
    print(f"")
    print(f"验证集结果:")
...
if __name__ == "__main__":
    result = extract_resnet_micro_error_samples()
    if result:
        print(f"\n✅ resnet_micro错误样本分析完成 (包含源文件名)!")
    else:
        print(f"\n❌ resnet_micro错误样本分析失败!")